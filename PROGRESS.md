...
## 2026-09-13 — Windows installers and download page

- Owner requested actual installers and removal of public Pro/pricing-planning text. Windows x64 v0.2.0 is now published at https://github.com/TakumaLee/watermark-tool/releases/tag/v0.2.0, built from `5bc8f81de097045120eadf763cff5bcf97c00455`. EXE is 59,851,103 bytes; MSI is 81,707,008 bytes. The release also carries checksums, the smoke report, FFmpeg notices, a source index and the FFmpeg core source archive. All 7 GitHub asset digests match the local files.
- Successful build: https://github.com/TakumaLee/watermark-tool/actions/runs/34744884088, Windows Server 2025 CI. Passed 173 frontend tests, production build, NSIS silent install/native-window startup, installed FFmpeg H.264 watermark overlay and ffprobe metadata check, and MSI administrative extraction/payload comparison. This is not full interactive editing or Windows 10/11 hardware validation. Installers are unsigned; WebView2 may require a runtime download. Python/Whisper/IOPaint extras are not bundled. Preview lag is unchanged; no macOS/Linux installer is claimed.
- First run `34744368934` built successfully but its raw app hash comparison rejected Tauri CLI 2.9.6's intentional NSS/MSI bundle marker. `scripts/verify-windows-payloads.py` now parses that PE pointer, validates each expected marker, and requires all remaining bytes to match. A valid synthetic pair passed; 6 corruption cases were rejected. Real payloads differ only at byte 10,770,424; normalized SHA256 `7eb3d64b1ad2ff35334d123a5a5be4b8e75426740a3b83b1d27cb0a89bc1677f`. Independent Claude reviews approved the build/publication gates and this bounded verification correction.
- Bundled Gyan FFmpeg 9.0.1 archive is pinned to SHA256 `fec81ae03971d9dd4be3ebe02e263bd2ec1d789483f931bdba5f5715e65da2e9`. Vendor LICENSE/README remain byte-identical; owned NOTICE differs only by Windows CRLF in the package. `docs/FFmpeg-SOURCES.md` records 42 vendor dependency entries (39 fixed source references, 3 unresolved development snapshots) and additional source-scope gaps. It does not claim a complete corresponding-source/rebuild bundle. Application license choice remains undecided.
- README and site now have Windows downloads and public source links; public Pro/pricing-planning copy is removed. Social card is updated. Local WebKit at 390/1440 px passed overflow, images, anchors, download-target and FAQ keyboard checks. Release files were published before deploying download availability claims. Existing Vercel project/team/configuration are retained. Production deployment `dpl_5M3sFKiV79BqLTwBwSGzq9pXmTWk` is READY at https://watermark-tool-m3kcckubl-nebula-proj.vercel.app, aliased to https://watermark-tool-ivory.vercel.app/. Anonymous HTTP verified all 8 public routes/assets against local bytes, the custom 404 and 6 excluded config/source paths, required headers and absence of Pro/pricing copy. Release page and all 7 asset URLs return anonymous HTTP 200; the Git tag resolves to the tested build commit.
- Task commits from `chore/windows-installers` are fast-forwarded to default `master`, without rewriting history. Release publication and website deployment are complete. The unrelated `.claude/scheduled_tasks.lock` deletion is preserved unstaged. Evidence is under ignored `output/playwright/installers-*`; local downloadable assets are under ignored `release/windows/`. The manual workflow retains read-only contents permission; the old competing tag trigger was removed.
...
- Publication branch: `chore/public-repository`. Default `master` was fast-forwarded from `42468ac` through the already-authorized preview correction and website commits to `848f6b5` (publication documentation/copy). No new application/performance changes, history rewrite, release installers or Git integration were introduced.
- Pre-publication disclosure review: Gitleaks scanned all 35 reachable local/remote commits (1.43 MB), with no findings. Historical paths were reviewed; no tracked credentials or private environment files were found. Existing offline CRC16 license generator/checker is part of the published app source, not a suitable basis for a future paid licensing system. The old scheduling lock contains only process/session metadata; its unrelated working-tree deletion was preserved and not staged. No tags, releases, issues/PRs (all states), wiki or discussions; 12 CI artifacts expired, 9 run logs returned 410 and one absent log returned 404. Independent Claude publication review approved the explicit authority, scope and disclosure checks.
- README now has real repository/website links and an actual screenshot, removes nonexistent installer/updater claims, and describes the unresolved license status instead of retaining a broken MIT badge/`LICENSE` link. No license was selected or legal rights asserted/revoked by this task.
- Website and social card now state source availability, link directly to GitHub, and keep installers/Pro pending. Production deployment: `dpl_8728qxXpdtwryQQ8PVjcG3eWb2P6`, https://watermark-tool-ifim6mqke-nebula-proj.vercel.app, aliased to https://watermark-tool-ivory.vercel.app/. Deployment configuration is unchanged.
- Verification: anonymous GitHub API reports `private: false`; repository, raw README, Releases and workflow badge return HTTP 200. Local WebKit at 390/1440 px verified the new source status/link without overflow. Anonymous production HTML and social image match the local files byte-for-byte and retain response headers. Production WebKit navigation timed out twice before the CTA interaction; anonymous HTTP checks succeeded, but the production browser click-through was not completed. This task changed documentation/static content only; the existing app checks were not repeated. Local review and verification evidence: `output/playwright/publication-*` (ignored, not published).
